...
export default function FreshNestFullERP() {
  ...
  function Supervisor() { return <div className="space-y-4"><Card className="bg-gradient-to-r from-[#07162a] to-slate-900 text-white"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-2xl font-black">Supervisor Firebase Web Sync</h3><p className="text-white/70">Website booking → Firebase sync → supervisor app workflow update ready.</p></div><button onClick={simulateWebsiteBooking} className="rounded-2xl bg-[#d4af37] px-4 py-2 font-black text-[#07162a]">Sync Website Booking</button></div></Card><div className="grid gap-4">{filtered.map((b) => <Card key={b.id || b.firebaseId}><div className="flex justify-between gap-3"><div><h3 className="text-xl font-black">{b.customer}</h3><p className="text-sm text-slate-500">{b.area} • {b.service}</p><p className="text-xs text-[#d4af37]">Supervisor: {b.supervisor}</p></div><div className="shrink-0"><Badge>{b.confirmed ? "Confirmed" : b.status}</Badge></div></div><div className="mt-4 grid gap-2 md:grid-cols-4">{["Pending", "On The Way", "Work Started", "Completed"].map((s) => <button key={s} onClick={() => updateBooking(b.id, { status: s, confirmed: s !== "Pending" })} className="rounded-2xl bg-slate-100 px-3 py-3 text-sm font-black hover:bg-[#d4af37]">{s}</button>)}</div><button onClick={() => openBooking(b)} className="mt-3 w-full rounded-2xl bg-[#07162a] px-4 py-3 font-black text-[#d4af37]">Open Full Workflow</button></Card>)}</div></div>; }
