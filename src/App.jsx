...
export default function FreshNestFullERP() {
  ...
  async function updateBooking(id, patch) {
    ...
    const current = bookings.find((b) => b.id === id || b.firebaseId === id);
    setBookings((prev) => prev.map((b) => (b.id === id || b.firebaseId === id ? { ...b, ...finalPatch } : b)));
    setSelected((old) => (old && (old.id === id || old.firebaseId === id) ? { ...old, ...finalPatch } : old));
    try {
      if (current?.firebaseId) {
        await updateDoc(doc(db, "bookings", current.firebaseId), { ...finalPatch, updatedAt: serverTimestamp() });
      }
    } catch (error) {
