...
const initialBookings = [
  { id: "FN-1001", customer: "Arun Kumar", phone: "9876543210", email: "arun@example.com", address: "12, North Street, Thillai Nagar", service: "Sofa Shampooing", serviceIcon: "🛋️", amount: 2200, status: "Not Started", paymentStatus: "Pending", payout: 1200, teamPayout: 900, fnMargin: 1000, status: "Pending", staff: "Unassigned", team: ["Ravi", "Manoj"], date: "2026-05-17", time: "10:30 AM", area: "Trichy - Thillai Nagar", source: "WhatsApp", notes: "4 seater sofa, shampoo required" },
...
function Card({ children, className = "" }) {
  return <div className={cn("rounded-3xl border border-slate-200/70 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900", className)}>{children}</div>;
}

function Badge({ children, tone }) {
  return <span className={cn("rounded-full px-3 py-1 text-xs font-semibold", statusStyle[tone] || "bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200")}>{children}</span>;
}
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
      <Card className="relative overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="rounded-2xl bg-[#0b1f3a] p-3 text-[#d4af37] dark:bg-white/10"><Icon size={24} /></div>
        </div>
      </Card>
    </motion.div>
  );
...
function DataInput({ value, onChange, className = "", type = "text", disabled = false }) {
  return <input disabled={disabled} type={type} value={value} onChange={(e) => onChange(e.target.value)} className={cn("w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-[#d4af37] disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-500 dark:border-slate-700 dark:bg-slate-950 dark:disabled:bg-slate-800", className)} />;
}

function ServiceVisual({ icon, name, photo, compact = false }) {
  return (
    <div className={cn("relative overflow-hidden rounded-3xl bg-slate-200 shadow-inner dark:bg-slate-800", compact ? "h-14 w-14" : "h-40 w-full")}>
      <img src={photo || SERVICE_PHOTOS[name] || SERVICE_PHOTOS["Deep Home Cleaning"]} alt={name} className="h-full w-full object-cover" />
      <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/10 to-transparent" />
      <div className={cn("absolute flex items-center justify-center rounded-2xl bg-white/90 shadow-lg", compact ? "bottom-1 left-1 h-8 w-8 text-lg" : "bottom-3 left-3 h-12 w-12 text-2xl")}>{icon || "🧹"}</div>
      {!compact && <div className="absolute bottom-4 left-20 right-3 text-xs font-bold text-white drop-shadow">{name}</div>}
    </div>
  );
}
...
export default function FreshNestAdminERP() {
  ...
  const [notifications, setNotifications] = useState([
    "New app booking synced: Water Tank Cleaning",
    "Low stock alert: Pest Spray",
    "Payment received for FN-1003",
  ]);
  ...
  function syncAppBooking() {
    const next = {
      id: `FN-${1000 + bookings.length + 1}`,
      customer: "New App Customer",
      phone: "9999988888",
      email: "appcustomer@example.com",
      address: "15, Demo Street, KK Nagar, Trichy",
      service: "Exterior Pressure Washing",
      serviceIcon: "🚿",
      amount: 2400,
      ...
      date: "2026-05-19",
      time: "11:00 AM",
      area: "Trichy - KK Nagar",
      source: "App",
      notes: "Auto synced from app booking simulation",
    };
    ...
    return (
      <div className="min-h-screen bg-[#07162a] p-5 text-white">
        <div className="mx-auto flex min-h-[90vh] max-w-6xl items-center justify-center">
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="grid w-full overflow-hidden rounded-[2rem] border border-white/10 bg-white/10 shadow-2xl backdrop-blur md:grid-cols-2">
            <div className="relative hidden p-10 md:block">
              <div className="absolute inset-0 bg-gradient-to-br from-[#d4af37]/30 to-transparent" />
              <div className="relative z-10">
                <div className="mb-8 inline-flex h-20 w-20 items-center justify-center rounded-3xl bg-white text-4xl shadow-xl">🧹</div>
                <h1 className="text-5xl font-black tracking-tight">FreshNest Admin ERP</h1>
                <p className="mt-4 max-w-md text-white/75">Urban-style cleaning business command center for bookings, staff, payments, stock, reports and customer support.</p>
                <div className="mt-10 grid gap-3">
                  {['Live booking sync', 'Editable service pricing', 'Payroll + attendance', 'Inventory alerts'].map((x) => <div key={x} className="flex items-center gap-3 rounded-2xl bg-white/10 p-3"><ShieldCheck className="text-[#d4af37]" /> {x}</div>)}
                </div>
              </div>
            </div>
            <div className="bg-white p-8 text-slate-950 dark:bg-slate-950 dark:text-white">
              <h2 className="text-3xl font-black">Admin Login</h2>
              <p className="mt-2 text-sm text-slate-500">Demo login. Click button to enter dashboard.</p>
              <div className="mt-8 space-y-4">
                <DataInput value="admin@freshnest.in" onChange={() => {}} />
                <DataInput type="password" value="freshnest123" onChange={() => {}} />
                <button onClick={() => setLoggedIn(true)} className="w-full rounded-2xl bg-[#0b1f3a] px-5 py-3 font-bold text-[#d4af37] shadow-lg transition hover:scale-[1.01]">Login to ERP</button>
              </div>
              <div className="mt-8 rounded-3xl bg-slate-100 p-4 dark:bg-slate-900">
                <p className="text-sm font-bold">Owners</p>
                <p className="mt-1 text-sm text-slate-500">Neethirajan & Selva Kumar</p>
              </div>
            </div>
          </motion.div>
        </div>
    ...
    return (
      <div className={cn(dark ? "dark" : "")}> 
        <div className="min-h-screen bg-slate-50 text-slate-950 dark:bg-[#06111f] dark:text-white">
          <aside className="fixed left-0 top-0 z-20 hidden h-full w-72 border-r border-slate-200 bg-[#07162a] p-4 text-white dark:border-slate-800 lg:block">
            <div className="mb-6 flex items-center gap-3 rounded-3xl bg-white/10 p-4">
              ...
              <div>
                <h1 className="font-black leading-tight">FreshNest</h1>
                <p className="text-xs text-[#d4af37]">Admin ERP</p>
              </div>
            </div>
            <nav className="space-y-1 overflow-y-auto pb-4">
              {nav.map(([name, Icon]) => (
                <button key={name} onClick={() => setActive(name)} className={cn("flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-left text-sm font-semibold transition", active === name ? "bg-[#d4af37] text-[#07162a]" : "text-white/75 hover:bg-white/10 hover:text-white")}>
                  <Icon size={18} /> {name}
                </button>
              ))}
            </nav>
          ...
          <main className="lg:pl-72">
            <header className="sticky top-0 z-10 border-b border-slate-200 bg-white/85 px-4 py-4 backdrop-blur dark:border-slate-800 dark:bg-[#06111f]/85">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h2 className="text-2xl font-black">{active}</h2>
                  <p className="text-sm text-slate-500">FN cleaning operations control panel</p>
                </div>
                <div className="flex items-center gap-2">
                  <div className="relative hidden sm:block">
                    <Search className="absolute left-3 top-2.5 text-slate-400" size={18} />
                    <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search bookings, customers..." className="w-72 rounded-2xl border border-slate-200 bg-white py-2 pl-10 pr-4 outline-none focus:border-[#d4af37] dark:border-slate-700 dark:bg-slate-900" />
                  </div>
                  <button onClick={syncAppBooking} className="rounded-2xl bg-[#0b1f3a] px-4 py-2 text-sm font-bold text-[#d4af37]"><Plus size={16} className="inline" /> App Sync</button>
                  <button onClick={() => setDark((v) => !v)} className={cn("flex items-center gap-2 rounded-2xl border px-3 py-2 text-sm font-bold transition", dark ? "border-[#d4af37] bg-[#d4af37] text-[#07162a]" : "border-slate-200 bg-white text-[#0b1f3a]")} title={dark ? "Bright mode" : "Dark mode"}>{dark ? <Sun size={18} /> : <Moon size={18} />} {dark ? 'Bright' : 'Night'}</button>
                  <div className="relative">
                    <button onClick={() => setShowNotifications(!showNotifications)} className="relative rounded-2xl border border-slate-200 p-2 dark:border-slate-700"><Bell /><span className="absolute -right-1 -top-1 h-4 w-4 rounded-full bg-[#d4af37] text-[10px] font-bold text-[#07162a]">{allNotifications.length}</span></button>
                    {showNotifications && (
                      <div className="absolute right-0 top-12 z-50 w-80 rounded-3xl border border-slate-200 bg-white p-3 shadow-2xl dark:border-slate-800 dark:bg-slate-950">
                        <div className="mb-2 flex items-center justify-between"><b>Notifications</b><button onClick={() => setShowNotifications(false)}><X size={16}/></button></div>
                        <div className="max-h-80 space-y-2 overflow-y-auto">
                          {allNotifications.map((n, i) => <div key={i} className="rounded-2xl bg-slate-100 p-3 text-sm dark:bg-slate-800">🔔 {n}</div>)}
                        </div>
                      </div>
                    )}
                  </div>
                  <button onClick={() => setLoggedIn(false)} className="rounded-2xl border border-slate-200 p-2 dark:border-slate-700"><LogOut /></button>
                </div>
              </div>
            </header>
  ...
  function Dashboard() {
    return <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard title="Total Revenue" value={`₹${totalRevenue.toLocaleString()}`} icon={IndianRupee} sub="Completed jobs only" />
        <StatCard title="Pending Bookings" value={pendingCount} icon={Clock3} sub="Need assignment" />
        <StatCard title="Completed Jobs" value={completedCount} icon={CheckCircle2} sub="This month" />
        <StatCard title="Low Stock Alerts" value={lowStock.length} icon={Package} sub="Inventory attention" />
      </div>
      <div className="grid gap-4 xl:grid-cols-3">
        <Card className="xl:col-span-2"><h3 className="mb-4 text-lg font-black">Live Booking Flow</h3><BookingTable compact /></Card>
        <Card><h3 className="mb-4 text-lg font-black">Notifications + Stock Alerts</h3><div className="space-y-3">{allNotifications.map((n, i) => <div key={i} className="rounded-2xl bg-slate-100 p-3 text-sm dark:bg-slate-800">🔔 {n}</div>)}</div></Card>
      </div>
    </div>;
  }
  ...
  function CalendarView() {
    ...
    const selectedList = bookings.filter((b) => b.date === selectedDate);

    const moveMonth = (step) => {
      const next = new Date(year, month + step, 1);
      setCalendarMonth(next);
      const firstDate = `${next.getFullYear()}-${String(next.getMonth() + 1).padStart(2, "0")}-01`;
      ...
      <div className="grid gap-5 xl:grid-cols-[1.3fr_.7fr]">
        <Card>
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
            <div>
              <h3 className="text-xl font-black">Monthly Booking Calendar</h3>
              <p className="text-sm text-slate-500">All months Prev / Next navigate pannalam</p>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => moveMonth(-1)} className="rounded-2xl border border-slate-200 p-2 hover:bg-slate-100 dark:border-slate-700 dark:hover:bg-slate-800"><ChevronLeft /></button>
              <div className="min-w-48 rounded-2xl bg-[#0b1f3a] px-4 py-2 text-center font-black text-[#d4af37]">{monthName}</div>
              <button onClick={() => moveMonth(1)} className="rounded-2xl border border-slate-200 p-2 hover:bg-slate-100 dark:border-slate-700 dark:hover:bg-slate-800"><ChevronRight /></button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-7 gap-2">
            {cells.map((day, i) => {
              if (!day) return <div key={`blank-${i}`} className="min-h-28 rounded-3xl bg-slate-50 dark:bg-slate-900/40" />;
              const dateKey = `${year}-${String(month + 1).padStart(2, "0")}-${String(day).padStart(2, "0")}`;
              const dayBookings = bookings.filter((b) => b.date === dateKey);
              const activeDate = selectedDate === dateKey;
              return (
                <button key={dateKey} onClick={() => setSelectedDate(dateKey)} className={cn("min-h-28 rounded-3xl border p-2 text-left transition hover:scale-[1.01]", activeDate ? "border-[#d4af37] bg-[#d4af37]/15" : "border-slate-200 bg-white hover:border-[#d4af37] dark:border-slate-800 dark:bg-slate-950")}>
                  <div className="flex items-center justify-between">
                    <span className="font-black">{day}</span>
                    {dayBookings.length > 0 && <span className="rounded-full bg-[#0b1f3a] px-2 py-1 text-[10px] font-bold text-[#d4af37]">{dayBookings.length} booking</span>}
                  </div>
                  <div className="mt-2 space-y-1">
                    {dayBookings.slice(0, 3).map((b) => <div key={b.id} className="truncate rounded-xl bg-slate-100 px-2 py-1 text-[11px] font-semibold dark:bg-slate-800">{b.serviceIcon || '🧹'} {b.customer}</div>)}
                    {dayBookings.length > 3 && <div className="text-[11px] text-slate-500">+{dayBookings.length - 3} more</div>}
                  </div>
                </button>
              );
            })}
        ...
        <Card>
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h3 className="text-lg font-black">Date Bookings</h3>
              <p className="text-sm text-slate-500">{selectedDate}</p>
            </div>
            <Badge tone={selectedList.length ? "Assigned" : "Pending"}>{selectedList.length} Jobs</Badge>
          </div>
          <div className="space-y-3">
            {selectedList.length === 0 && <div className="rounded-3xl bg-slate-100 p-5 text-center text-sm text-slate-500 dark:bg-slate-800">Intha date la bookings illa da.</div>}
            {selectedList.map((b) => (
              <button key={b.id} onClick={() => setSelectedBooking(b)} className="w-full rounded-3xl border border-slate-200 p-4 text-left transition hover:border-[#d4af37] hover:shadow-md dark:border-slate-800">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-3"><ServiceVisual compact icon={b.serviceIcon} name={b.service} photo={SERVICE_PHOTOS[b.service]} /><h4 className="font-black">{b.customer}</h4></div>
                    <p className="mt-1 text-sm text-slate-500">{b.service}</p>
                    <p className="mt-1 text-xs text-slate-500">{b.time} • {b.area}</p>
                  </div>
                  <Badge tone={b.status}>{b.workStatus || b.status}</Badge>
                </div>
              </button>
            ))}
          </div>
        ...
        {selectedBooking && <CustomerDrawer booking={selectedBooking} onClose={() => setSelectedBooking(null)} />}
      </div>
    );
  }
  ...
  function CustomerDrawer({ booking, onClose }) {
    ...
    return (
      <div className="fixed inset-0 z-50 flex justify-end bg-black/50 p-3 backdrop-blur-sm">
        <motion.div initial={{ x: 420, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="h-full w-full max-w-xl overflow-y-auto rounded-[2rem] bg-white p-5 shadow-2xl dark:bg-slate-950">
          <div className="sticky top-0 z-10 mb-4 flex items-center justify-between bg-white/90 pb-3 backdrop-blur dark:bg-slate-950/90">
            <div>
              <h3 className="text-2xl font-black">{booking.customer}</h3>
              <p className="text-sm text-slate-500">{booking.id} full booking drawer</p>
            </div>
            <button onClick={onClose} className="rounded-2xl border border-slate-200 p-2 dark:border-slate-800"><X /></button>
          </div>

          <div className="grid gap-4">
            <Card className="bg-slate-50 dark:bg-slate-900">
              <div className="flex items-start gap-4">
                <ServiceVisual compact icon={booking.serviceIcon} name={booking.service} photo={SERVICE_PHOTOS[booking.service]} />
                <div className="flex-1">
                  <h4 className="font-black">Customer Full Details</h4>
                  <p className="mt-2 text-sm"><UserRound size={15} className="mr-1 inline"/> {booking.customer}</p>
                  <p className="mt-1 text-sm"><Phone size={15} className="mr-1 inline"/> {booking.phone}</p>
                  <p className="mt-1 text-sm"><MapPin size={15} className="mr-1 inline"/> {booking.address || booking.area}</p>
                </div>
              </div>
              <button onClick={() => openWhatsApp(booking)} className="mt-4 w-full rounded-2xl bg-emerald-600 px-4 py-3 font-bold text-white"><MessageCircle size={18} className="inline"/> WhatsApp Customer</button>
            </Card>

            <Card>
              <h4 className="mb-3 flex items-center gap-2 font-black"><BriefcaseBusiness className="text-[#d4af37]"/> Work / Service Details</h4>
              <div className="grid gap-3 text-sm">
                <div className="rounded-2xl bg-slate-100 p-3 dark:bg-slate-800"><b>Service:</b> <span className="text-xl">{booking.serviceIcon}</span> {booking.service}</div>
                <div className="rounded-2xl bg-slate-100 p-3 dark:bg-slate-800"><b>Date & Time:</b> {booking.date} • {booking.time}</div>
                <div className="rounded-2xl bg-slate-100 p-3 dark:bg-slate-800"><b>Notes:</b> {booking.notes}</div>
                <div className="rounded-2xl bg-slate-100 p-3 dark:bg-slate-800"><ImageIcon size={15} className="mr-1 inline"/><b>Service Image Icon:</b> {booking.serviceIcon || '🧹'} demo icon ready</div>
              </div>
            </Card>

            <Card>
              <h4 className="mb-3 font-black">Status Control</h4>
              <select value={booking.workStatus || booking.status} onChange={(e) => { updateBooking(booking.id, 'workStatus', e.target.value); setSelectedBooking({ ...booking, workStatus: e.target.value }); }} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 font-bold outline-none dark:border-slate-700 dark:bg-slate-900">
                {['Not Started','Assigned','Live','Completed'].map((x) => <option key={x}>{x}</option>)}
              </select>
              <div className="mt-3"><Badge tone={booking.status}>{booking.workStatus || booking.status}</Badge></div>
            </Card>

            <Card>
              <h4 className="mb-3 flex items-center gap-2 font-black"><CreditCard className="text-[#d4af37]"/> Payment Status</h4>
              <div className="grid gap-3 md:grid-cols-2">
                <div className="rounded-2xl bg-slate-100 p-4 dark:bg-slate-800"><p className="text-xs text-slate-500">Customer Amount</p><p className="text-2xl font-black">₹{booking.amount}</p></div>
                <div className="rounded-2xl bg-slate-100 p-4 dark:bg-slate-800"><p className="text-xs text-slate-500">Payment</p><p className="text-xl font-black">{booking.paymentStatus}</p></div>
              </div>
            </Card>

            <Card>
              <h4 className="mb-3 flex items-center gap-2 font-black"><HandCoins className="text-[#d4af37]"/> Payout Details</h4>
              <div className="grid gap-3 md:grid-cols-3">
                <div className="rounded-2xl bg-slate-100 p-4 dark:bg-slate-800"><p className="text-xs text-slate-500">Total Payout</p><p className="text-xl font-black">₹{booking.payout}</p></div>
                <div className="rounded-2xl bg-slate-100 p-4 dark:bg-slate-800"><p className="text-xs text-slate-500">Team Payout</p><p className="text-xl font-black">₹{booking.teamPayout}</p></div>
                <div className="rounded-2xl bg-slate-100 p-4 dark:bg-slate-800"><p className="text-xs text-slate-500">FN Margin</p><p className="text-xl font-black text-emerald-600">₹{booking.fnMargin}</p></div>
              </div>
              <div className="mt-3 rounded-2xl bg-[#0b1f3a] p-4 text-sm text-white"><b>Team:</b> {(booking.team || []).join(', ')} • <b>Balance payout:</b> ₹{payoutBalance}</div>
            </Card>
          </div>
        </motion.div>
      </div>
  ...
  function SupervisorApp() {
    ...
    return (
      <div className="mx-auto max-w-5xl space-y-5">
        <Card className="bg-[#07162a] text-white">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h3 className="text-2xl font-black">Supervisor Mobile App</h3>
              <p className="text-sm text-white/70">Ithu staff/supervisor side simulation. Inga update panna admin dashboard, calendar, booking drawer la live sync aagum.</p>
            </div>
            <Badge tone="Assigned">Live Sync ON</Badge>
          </div>
        </Card>

        <div className="grid gap-4 md:grid-cols-2">
          {supervisorJobs.map((b) => (
            <Card key={b.id} className="overflow-hidden p-0">
              <ServiceVisual icon={b.serviceIcon} name={b.service} photo={SERVICE_PHOTOS[b.service]} />
              <div className="p-5">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h4 className="text-xl font-black">{b.customer}</h4>
                    <p className="text-sm text-slate-500">{b.id} • {b.service}</p>
                    <p className="mt-1 text-sm text-slate-500">{b.date} • {b.time}</p>
                  </div>
                  <Badge tone={b.status}>{b.workStatus || b.status}</Badge>
                </div>
                <div className="mt-4 grid grid-cols-2 gap-2">
                  <button onClick={() => supervisorUpdate(b.id, 'On The Way')} className="rounded-2xl bg-blue-600 px-3 py-3 text-sm font-bold text-white">On The Way</button>
                  <button onClick={() => supervisorUpdate(b.id, 'Live')} className="rounded-2xl bg-amber-500 px-3 py-3 text-sm font-bold text-[#07162a]">Live</button>
                  <button onClick={() => supervisorUpdate(b.id, 'Completed')} className="rounded-2xl bg-emerald-600 px-3 py-3 text-sm font-bold text-white">Completed</button>
                  <button onClick={() => supervisorUpdate(b.id, b.workStatus || 'Completed', 'Paid')} className="rounded-2xl bg-[#0b1f3a] px-3 py-3 text-sm font-bold text-[#d4af37]">Paid</button>
                </div>
                <div className="mt-4 rounded-2xl bg-slate-100 p-3 text-sm dark:bg-slate-800">
                  <b>Payment:</b> {b.paymentStatus} • <b>FN Margin:</b> ₹{b.fnMargin}
                </div>
              </div>
            </Card>
          ))}
  ...
  function Complaints() { return <SimpleEditableTable title="Complaints Management" rows={complaints} setRows={setComplaints} columns={[{key:'customer',label:'Customer',edit:true},{key:'issue',label:'Issue',edit:true},{key:'priority',label:'Priority',edit:true},{key:'status',label:'Status',badge:true}]} />; }

  function Inventory() { return <SimpleEditableTable title="Inventory + Stock Alerts" rows={inventory} setRows={setInventory} columns={[{key:'item',label:'Item',edit:true},{key:'stock',label:'Stock',edit:true,num:true},{key:'min',label:'Minimum',edit:true,num:true},{key:'unit',label:'Unit',edit:true}]} />; }

  function Expenses() { return <SimpleEditableTable title="Expenses" rows={expenses} setRows={setExpenses} columns={[{key:'title',label:'Title',edit:true},{key:'amount',label:'Amount',edit:true,num:true},{key:'date',label:'Date',edit:true},{key:'category',label:'Category',edit:true}]} />; }

  function Attendance() { return <SimpleEditableTable title="Staff Attendance" rows={staff} setRows={setStaff} columns={[{key:'name',label:'Name'},{key:'role',label:'Role'},{key:'status',label:'Status',edit:true},{key:'jobs',label:'Jobs',edit:true,num:true}]} />; }

  function Payroll() { return <SimpleEditableTable title="Payroll Editable" rows={staff} setRows={setStaff} columns={[{key:'name',label:'Staff'},{key:'role',label:'Role'},{key:'salary',label:'Salary',edit:true,num:true},{key:'advance',label:'Advance',edit:true,num:true}]} />; }
